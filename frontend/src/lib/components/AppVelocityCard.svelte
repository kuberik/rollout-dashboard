	...
	<div class="flex items-start gap-3 px-5 pt-4">
		...
		<div class="flex min-w-0 flex-1 flex-col">
			<div class="flex min-w-0 items-baseline gap-2">
				<span class="truncate text-base font-semibold text-gray-900 dark:text-white">{name}</span>
				{#if worstStuck}<StuckBadge reason={worstStuck} size="xs" />{/if}
			</div>
			{#if title && title !== name}<span class="truncate text-[11px] text-gray-400 dark:text-gray-500">{title}</span>{/if}
		</div>
