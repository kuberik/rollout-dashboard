...
<div class="apps-cq mx-auto w-full max-w-7xl px-4 py-6 sm:px-6">
	...
	<div class="mb-5 flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-1">
		...
		{#if query.isLoading}
			<!-- ⭐ THE HEAD BAND IS RESERVED FIRST, IN THE SAME `mb-5` ROW THE
			     REAL SENTENCE SITS IN — not a separate block above it.
			     (load-state audit finding 10: the old skeleton had no head
			     band at all, so the amber banner and the list both jumped up
			     when the real rollup appeared.) See `HeadBandSkeleton`'s own
			     note; this inlines its two bars rather than the whole
			     component because the `h1` here needs to share this exact
			     `flex items-baseline` row, not a second one. -->
			<span class="skel-block h-7 w-6" aria-hidden="true"></span>
			<span class="skel-block h-3.5 w-72" aria-hidden="true"></span>
		{:else if !query.isError && appRows.length > 0}
	...
	{#if query.isLoading}
		...
		<!--
			⭐ THE SAME `.apps-split` / `.apps-panel` / `.apps-row` THE LOADED
			PAGE USES, NOT A GENERIC LIST. (2026-09-04, load-state audit
			finding 10) The old skeleton was one 1201-wide bordered box of six
			rows whose `hidden sm:block` cells collapsed at 390 (leaving
			almost nothing on screen while loading), while the loaded page is
			a head band, an 865-wide list card and a 320-wide rail. The row
			itself reuses this file's own `.apps-row`/`.apps-id`/`.apps-fleet`/
			`.apps-act`/`.apps-lead`/`.apps-chev`/`.apps-desc`/`.apps-mark`
			classes — scoped to this component, so the SAME container queries
			that reflow a real row at 720/840px reflow this one identically,
			including the 390 stacked form the old skeleton never drew.

			⛔ NO BANNER SKELETON HERE. `blocker` is derived entirely from
			this one query's own data (unlike rollout detail's multi-query
			Overview), so on a genuinely cold load the block state is not
			knowable yet — principle 7's "otherwise" branch: the banner's
			INSERTION above the list, once data arrives, is the one thing
			this composition still allows to move.
		-->
		<div class="apps-split">
									...
									<span class="apps-id flex min-w-0 items-center gap-3">
										...
										<span class="flex min-w-0 flex-1 flex-col gap-1.5">
											<span class="flex min-w-0 items-baseline gap-2">
												...
												<span class="skel-block apps-desc h-2.5 w-36"></span>
											</span>
											<span class="skel-block h-2.5 w-56"></span>
										</span>
									</span>
									<span class="apps-fleet apps-mark flex min-w-0 items-center">
										<span class="skel-block h-3.5 w-28"></span>
									</span>
									<span class="apps-act apps-mark flex min-w-0 items-center">
										<span class="skel-block h-3.5 w-16"></span>
									</span>
									<span class="apps-lead apps-mark flex min-w-0 items-center">
										<span class="skel-block h-3.5 w-14"></span>
									</span>
									<span class="apps-chev flex items-center justify-end">
										<span class="skel-block h-4 w-4"></span>
									</span>
								</li>
							{/each}
						</ul>
			...
			<!-- The rail — same two cards, same order, as the loaded branch
			     below. -->
			<div class="min-w-0 space-y-4">
				<CardSkeleton titleWidth="w-28" rollupWidth="w-24" rows={3} rowHeight={20} />
				<CardSkeleton titleWidth="w-32" rollupWidth="w-16" rows={3} rowHeight={28} />
			</div>
		</div>
