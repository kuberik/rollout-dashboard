...
// TestGitHubCommitPulls_ShaUnprocessable422ScopeSha mirrors
// TestGitHubCommit_ShaUnprocessable422ScopeSha: GitHub answers an
// unresolvable sha with 422 "No commit found for SHA" when resolving the
// short sha via repos/{o}/{r}/commits/{sha}, not 404 — this must still land
// as a 404 scope=sha, not the raw 502 the live bug reported.
func TestGitHubCommitPulls_ShaUnprocessable422ScopeSha(t *testing.T) {
	r := setupGitHubPullsTest(t, []client.Object{
		rolloutWithSource("team-a", "app-1", "https://github.com/octo/repo"),
	})
	const short = "bf5be49"
	...
	w := doGitHubPullsRequest(r, "/api/github/repos/octo/repo/commits/"+short+"/pulls", "ghu_unprocessable_token")
	if w.Code != http.StatusNotFound {
		t.Fatalf("status = %d, want 404 (body: %s)", w.Code, w.Body.String())
	}
	if !contains(w.Body.String(), `"scope":"sha"`) {
...
// TestGitHubCommitPulls_UpstreamServerError502 confirms a genuine upstream
// failure still surfaces as a 502, not swallowed into the 404
// reinterpretation above.
func TestGitHubCommitPulls_UpstreamServerError502(t *testing.T) {
	r := setupGitHubPullsTest(t, []client.Object{
		rolloutWithSource("team-a", "app-1", "https://github.com/octo/repo"),
	})
	const fullSha = "0123456789abcdef0123456789abcdef01234567"
	...
	w := doGitHubPullsRequest(r, "/api/github/repos/octo/repo/commits/"+fullSha+"/pulls", "ghu_servererror_token")
